...
export default function GeofenceConfiguration() {
  ...
  const filteredGeofences = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (geofence) =>
          geofence.geofence_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          geofence.tag?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          getGeofenceTypeString(geofence.geofence_type).toLowerCase().includes(searchTerm.toLowerCase()) ||
          geofence.id.toString().includes(searchTerm) ||
          geofence.location_id?.toString().includes(searchTerm),
      )
    }
  ...
  const handleSearchChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim()) {
      try {
        const searchResults = await searchGeofences(value.trim())
  ...
  return (
    <>
      <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 pt-4 pb-2">
        ...
        {view === "list" ? (
          ...
          <div className="flex-1 relative">
            ...
            <div className="absolute top-4 right-4 z-10">
              <Card className="w-80 shadow-lg dark:bg-gray-800 dark:border-gray-700">
