...
import type { GeofenceMapProps } from "../../../types/geofence/gconfig"
...
export default function GeofenceMap({
          ...
          layer.bindPopup(`
            <div class="overflow-hidden rounded-lg" style="min-width: 220px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
              <div style="background-color: ${circleColor}; padding: 10px 12px; color: white; display: flex; justify-content: space-between; align-items: center;">
                <div style="font-weight: bold; font-size: 14px;">${geofence.name}</div>
              ...
              <div style="padding: 12px; background-color: white;">
                <div style="margin-bottom: 8px; display: flex; align-items: center;">
                  ...
                  <span style="margin-left: 6px; color: #4b5563; font-size: 13px;">${geofence.location || "No location specified"}</span>
                </div>
  ...
  const removePolygonPoint = (index: number) => {
    ...
    if (index >= 0 && index < polygonMarkers.length) {
      mapInstance.polygonEditLayerGroup.removeLayer(polygonMarkers[index])
      ...
      const newPolygonMarkers = points.map((point, idx) => {
        ...
        marker.bindPopup(`
          <button class="px-2 py-1 bg-red-500 text-white text-xs rounded" 
            onclick="document.dispatchEvent(new CustomEvent('deletePolygonPoint', {detail: ${idx}}))">
            Remove Point
          </button>
        `)
  ...
  return (
    <div className="h-full relative">
      {/* Map container */}
      <div ref={mapContainerRef} className="h-full w-full z-5" />
      ...
      {editingGeofence && editingGeofence.type === "polygon" && (
        <div className="absolute top-4 left-40 z-50">
          ...
          {drawingMode && (
            <div className="mt-2 bg-white p-2 rounded-md shadow-md text-xs">
      ...
      {selectedGeofence && !editingGeofence && (
        <div className="absolute top-20 left-4 z-10">
          <Button onClick={() => onEditGeofence(selectedGeofence)} className="flex items-center gap-2">
      ...
      {!leafletLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-80 z-50">
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 border-4 border-t-red-500 border-red-200 rounded-full animate-spin"></div>
