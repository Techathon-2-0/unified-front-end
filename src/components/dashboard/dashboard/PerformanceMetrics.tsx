...
const PerformanceMetrics = () => {
  ...
  // Calculate on-time delivery based on actual vehicle performance
  const onTimeVehicles = vehicles.filter(v => 
    v.status === "Active" && v.speed > 0 && v.speed <= 80
  ).length
  const onTimeDelivery = totalVehicles > 0 ? Math.round((onTimeVehicles / totalVehicles) * 100) : 0

  const calculateOverallPerformance = () => {
    const utilizationScore = fleetUtilization * 0.3
    const efficiencyWeight = efficiencyScore * 0.25
    const onTimeWeight = onTimeDelivery * 0.25
