...
import type { EntityDrawerProps, Vendor } from "../../../types/manage/entity"
          ...
          <div className="flex items-center justify-center min-h-screen p-4">
            ...
            >
              ...
              <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-200px)] pb-20 sm:pb-6">
                ...
                >
                  <div>
                    <Label htmlFor="vehicleNumber" className="text-sm font-medium text-gray-700 mb-1 flex items-center">
                    ...
                    <Input
                      ...
                      className={errors.vehicleNumber ? "border-red-500 ring-1 ring-red-500" : ""}
                  ...
                  <div>
                    <Label className="text-sm font-medium text-gray-700 mb-3 block">
                      Vendors {!entity && <span className="text-gray-500 text-xs">(Optional - can be set later)</span>}
                    </Label>
                    <div className="space-y-2 max-h-40 overflow-y-auto border rounded-md p-3">
                      {availableVendors.map((vendor) => (
                        <div key={vendor.id} className="flex items-center space-x-2">
                          ...
                          />
                          <Label htmlFor={`vendor-${vendor.id}`} className="text-sm text-gray-700 cursor-pointer">
                            {vendor.name}
                          </Label>
                        </div>
                      ))}
                  ...
                  {entity && (
                    <div className="bg-gray-50 rounded-lg p-4">
                      <h3 className="text-sm font-medium text-gray-700 mb-2">Entity Information</h3>
                      <div className="grid grid-cols-1 gap-2 text-sm">
                        <div>
                          <span className="text-gray-500">Entity ID:</span>
                          <span className="ml-2 font-medium text-black">{entity.id}</span>
                        </div>
                        {selectedVendors.length > 0 && (
                          <div>
                            ...
                            <div className="ml-2 mt-1 space-y-2">
                                ...
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {selectedVendors
                                      ...
                                      .map((vendor) => (
                                        <span key={vendor.id} className="text-black text-xs bg-green-100 px-2 py-1 rounded">
                                          {vendor.name}
                                        </span>
                                      ))}
                                  </div>
                              ...
                              {selectedVendors.filter(vendor => vendor.status === false).length > 0 && (
                                <div>
                                  <div className="text-xs font-medium text-red-700 mb-1">Inactive:</div>
                                  <div className="flex flex-wrap gap-1">
                                    {selectedVendors
                                      .filter(vendor => vendor.status === false)
                                      .map((vendor) => (
                                        <span key={vendor.id} className="text-black text-xs bg-red-100 px-2 py-1 rounded">
                                          {vendor.name}
                                        </span>
                                      ))}
                                  </div>
