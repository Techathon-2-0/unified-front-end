...
import type { TrailControlsProps } from "../../types/trail/trail_type"
      ...
      <div className="flex items-start">
        <div className="flex items-center space-x-2 w-1/3">
          ...
          <div className="overflow-hidden">
            <p className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{location || "Unknown Location"}</p>
        ...
        <div className="flex-1 flex flex-col items-center">
          <div className="flex items-center space-x-4 mb-2">
            <button onClick={handleSkipBack} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-gray-300">
              <SkipBack size={20} />
            </button>
            ...
            <motion.button
              whileTap={{ scale: 0.95 }}
              onClick={onPlayPause}
              className="w-10 h-10 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-600 dark:text-gray-300"
            >
              {isPlaying ? <Pause size={20} /> : <Play size={20} />}
            </motion.button>

            <button onClick={handleSkipForward} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-gray-300">
              <SkipForward size={20} />
            </button>
            ...
            <select
              ...
              className="bg-gray-100 dark:bg-gray-700 text-sm rounded-md px-2 py-1 dark:text-gray-300 dark:border-gray-600"
            >
        ...
        <div className="w-1/3 flex justify-end">
          <div className="grid grid-cols-2 gap-4 text-right">
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">Distance (Kms)</p>
              <p className="text-xl font-bold dark:text-white">{currentDistance.toFixed(2)}</p>
            </div>

            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">Time (Hrs : Mins)</p>
              <div className="flex items-center justify-end">
                <p className="text-xl font-bold dark:text-white">{currentTime}</p>
              </div>
            </div>
            ...
            <div className="col-span-2">
              ...
              <p className="text-3xl font-bold dark:text-white">
                {currentSpeed} <span className="text-sm font-normal dark:text-gray-300">Km/h</span>
              </p>
            </div>
          </div>
        </div>
